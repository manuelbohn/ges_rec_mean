...
data%>%
  mutate(responses = as.character(responses),
         responses= substr(responses, 1, nchar(responses)-2))
...
## inhaltlich relevant (erst überprüfen wie viele Fälle) 
...
3.2. dependent variable 'length' (hier keine binäre Codierung) -> Poisson Verteilung
```{r}
...
# m4_l_p (condition als einziger Prädikter, Rest random effects) 
...
weigths_length_poisson <-  model_weights(m1_l_p, m2_l_p, m3_l_p, m4_l_p, m5_l_p, weights = "waic")%>%
  as_data_frame(rownames = "Predictors")%>%
  rename(weight = value)

# model comparison 
loo_compare(m1_l_p, m2_l_p, m3_l_p, m4_l_p, m5_l_p, criterion = "waic")%>%
  as_data_frame(rownames = "Predictors") %>%
  left_join(weigths_length_poisson)%>%
  mutate(
...
# model comparison 
weigths_length_poisson <-  model_weights(m1_l_nb, m2_l_nb, m3_l_nb, m4_l_nb, m5_l_nb, weights = "waic")%>%
  as_data_frame(rownames = "Predictors")%>%
  rename(weight = value)
...
x <- posterior_samples(m1_fc, "^b")%>%
  mutate(movement_hol = plogis(b_Intercept),
         movement_seq = plogis(b_Intercept + b_conditionseq),
         number_hol = plogis(b_Intercept+ b_propertynumber),
         number_seq = plogis(b_Intercept+ b_conditionseq + b_propertynumber + `b_conditionseq:propertynumber`),
         size_hol = plogis(b_Intercept+ b_propertysize),
         size_seq = plogis(b_Intercept+ b_conditionseq + b_propertysize + `b_conditionseq:propertysize`))%>%
  select(movement_hol,movement_seq,number_hol,number_seq,size_hol,size_seq)%>%
  gather(condition, y)%>%
  separate(condition, sep = "_", into = c("property","condition"))%>%
  group_by(property, condition)%>%
  summarise(map = estimate_mode(y),
            lci = hdi_lower(y),
            uci = hdi_upper(y))

ggplot(x, aes(x = condition, y = map, col= property)) +
  geom_pointrange(aes(y = map, ymin = lci, ymax = uci), pch = 1, position = position_dodge(width = .5))+
  labs(x="condition",y="Proportion correct feature")+
  theme_few() +
  ylim(-0.05,1.05)
...
data_wordcloud <- data_file %>%
  ...
  group_by(object, predicate, condition, property, class, value)%>%
  summarise(freq = sum(freq))


## TO DO: Rechtschreibfehler korrigieren

### ?? nachdem Kleinschreibung geändert wurde müssten die Wörter, die jetzt gleich sind, als ein value notiert werden (in der Zeile muss doe freq der beiden Wörter addiert werden und nur ein type (sollten identisch sein) verwendet werden)
data_words <- read.csv2("Words_data_codiert.csv")%>%
  select(-X) %>%
  mutate (value = tolower(value)) %>%
  group_by(value) %>%
...
## vor Häufigkeiten nur Kleinschreibung!! (tolower)
...
4.2. Wordclouds for objects (objects/animals) (sepearate for hol versus seq) (für jedes Objekt sepearat)
```{r}
## par(mfrow=c(2,2))
...
4.3. wordclouds for features (movement/number/size) (sepearate for hol versus seq) (für jedes feature seperat, Rechtschreibung)
```{r}
## movement (hol vs seq)
